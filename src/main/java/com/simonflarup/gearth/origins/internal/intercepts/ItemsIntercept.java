...
class ItemsIntercept extends AbstractIntercept {
    static void onItemsAdd(ShockPacketIncoming packet, OHContext context) {
        OHItem item = new OHItem(packet);
        context.getEventSystem().post((OnItemAddedEvent) () -> item);
    }

    static void onItemsUpdate(ShockPacketIncoming packet, OHContext context) {
        OHItem item = new OHItem(packet);
        context.getEventSystem().post((OnItemUpdatedEvent) () -> item);
    }

    static void onItemsRemove(ShockPacketIncoming packet, OHContext context) {
        int headerBytes = 2;
        ...
        context.getEventSystem().post((OnItemRemovedEvent) () -> id);
    }

    static void onItems(ShockPacketIncoming packet, OHContext context) {
        if (packet.length() > 2) {
            OHItem[] items = OHItem.parse(packet);
            context.getEventSystem().post((OnItemsLoadedEvent) () -> items);
        }
    }
}
